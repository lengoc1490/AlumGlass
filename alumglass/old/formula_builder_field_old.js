...
function _getFrmDocJsonForScope(scope) {
  ...
  if (typeof aluglass?.formula?._serializeDoc === "function") {
    return aluglass.formula._serializeDoc(scope?.current_doctype, scope?.current_docname);
  }
...
function _esc(str) {
  ...
  if (typeof aluglass?.formula?._esc === "function") return aluglass.formula._esc(str);
  return String(str || "")
    .replace(/&/g,"&amp;").replace(/</g,"&lt;")
    .replace(/>/g,"&gt;").replace(/"/g,"&quot;");
}
...
function _ensureFieldPatchCSS() {
  ...
  s.textContent = `
    ...
    .afb-grid-float-popup {
      ...
      overflow: visible;
      /* vị trí + kích thước được set bằng JS */
    }
...
function _buildContext({ frm, childTableField, cdn }) {
  if (aluglass.formula._ContextBuilder?.build) {
    return aluglass.formula._ContextBuilder.build({ frm, childTableField, cdn, liveCtx: null });
  }
  const SKIP = new Set(["Section Break","Column Break","Tab Break","Heading","Button",
...
function _registerHoverProvider(monacoEditor, contextFn) {
  ...
  const langId = monacoEditor.getModel()?.getLanguageId?.() || "aluglass-formula";
...
function _buildWrapHTML({ fieldname, cdnAttr, language, height, showToolbar, showPreview }) {
  ...
  const toolbarHtml = showToolbar ? `
    <div class="afb-fp-toolbar">
      ...
      <button class="aluglass-btn fp-copy" title="Copy">📋</button>
    </div>` : "";
...
function _createInlineMonaco({ container, initialValue, cfg, contextFn, onChangeFn, scopeFn, previewEl, openDialogFn }) {
  ...
  monacoEditor = monaco.editor.create(container, {
    ...
    theme               : window._afbLightTheme ? "aluglass-light" : "vs",
  ...
  const model = monacoEditor.getModel();
  aluglass.formula.CompletionRegistry.ensureLanguage(cfg.language);
  aluglass.formula.CompletionRegistry.register(model, _buildCompletionHandler(monacoEditor, contextFn), cfg.language);
  ...
  return {
    ...
    dispose: () => {
      ...
      if (model) aluglass.formula.CompletionRegistry.unregister(model);
...
// ═══════════════════════════════════════════════════════════════════════════════
aluglass.formula.patchField = function(frm, fieldname, opts = {}) {
  ...
  $fw.find(".afb-fp-wrap, .aluglass-field-patch-wrap").remove();
...
// ═══════════════════════════════════════════════════════════════════════════════
aluglass.formula.patchChildField = function(frm, childTableField, cdn, fieldname, opts = {}) {
  ...
  const cfgDefaults  = fieldMeta ? {} : { language: opts.language || "aluglass-formula" };
...
// ═══════════════════════════════════════════════════════════════════════════════
aluglass.formula.patchFormFields = function(frm, globalOpts = {}, fieldOpts = {}) {
  ...
  Object.entries(frm.fields_dict).forEach(([fieldname, field]) => {
    ...
    aluglass.formula.patchField(frm, fieldname, Object.assign({}, globalOpts, fieldOpts[fieldname] || {}));
  });
};

let _globalPatchEnabled  = false;
let _originalFormRefresh = null;
aluglass.formula.enableGlobalPatch = function(defaultOpts = {}, filterFn = null) {
  ...
  proto.refresh = function(...args) {
    ...
    setTimeout(() => {
      ...
      Object.entries(this.fields_dict).forEach(([fieldname, field]) => {
        ...
        aluglass.formula.patchField(this, fieldname, defaultOpts);
      });
    }, 0);
    return result;
  };
...
aluglass.formula.disableGlobalPatch = function() {
...
aluglass.formula.disposePatchedField = function(frm, fieldname) {
  _patchedEditors[_editorKey(frm, fieldname)]?.dispose();
};
aluglass.formula.disposeFormPatches = function(frm) {
  const prefix = `${frm.doctype}::`;
  Object.keys(_patchedEditors).forEach(key => {
    if (key.startsWith(prefix)) _patchedEditors[key].dispose();
  });
};
aluglass.formula.disposeChildPatch = function(frm, childTableField, cdn, fieldname) {
  _patchedEditors[`${frm.doctype}::${childTableField}::${cdn}::${fieldname}`]?.dispose();
};
aluglass.formula.syncChildFieldValue = function(frm, childTableField, cdn, fieldname) {
...
//
// Sử dụng:
//   aluglass.formula.initGridField(frm, "items", "custom_item_formula", {
...
// ═══════════════════════════════════════════════════════════════════════════════
aluglass.formula.initGridField = function(frm, gridField, fieldname, opts = {}) {
    ...
    function _bindExpandedRow(cdn, rowEl) {
        ...
        aluglass.formula.patchChildField(frm, gridField, cdn, fieldname, {
            language    : cfg.language,
            height      : cfg.height,
