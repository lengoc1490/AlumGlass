Tôi sẽ cung cấp **toàn bộ code sửa lỗi chi tiết** cho cả hai tài liệu, theo đúng thứ tự yêu cầu. Mỗi mục đều ghi rõ **file nào, dòng nào (hoặc vị trí nào)**, và **code mới thay thế**. Bạn chỉ cần copy‑paste vào đúng chỗ.

---

# I. SỬA THEO `BAN-FULL-A-Z.md` (các phần B → H)

## Phần B – Fix N+1 composite pricing

**File:** `alumglass/engine/bom_orchestrator.py`

### B.1 – Thay thế hàm `_fetch_composite_prices_via_fb` (khoảng dòng 1095–1158)
...
### B.2 – Thay thế `_fetch_composite_prices` và `_match_composite_price` (dòng ~1160–1238)
...
## Phần C – Hardcode `"KINH"`

### C.1 – Tạo helper mới

**File mới:** `alumglass/al_bom_engine/glass_group_resolver.py` (nếu chưa có, tạo với nội dung dưới; nếu đã có, thêm hàm `fetch_glass_category_codes` vào đầu)
...
### C.2 – Sửa `bom_orchestrator.py` – trong `b2_prefetch_master_data`

Tìm đoạn "Batch query #4" và thêm dòng gán `self._glass_categories`:
...
```python
        ...
        self._glass_categories = fetch_glass_category_codes(cat_codes)   # ← THÊM DÒNG NÀY
```

### C.3 – Thay thế 2 chỗ so sánh `== "KINH"` trong `bom_orchestrator.py`

**Dòng ~721 và ~781:** sửa từ
...
### C.4 – Sửa `api/__init__.py` dòng ~729
...
### D.1 – Thêm field vào `AL Pricing Dimension`

**File JSON:** `alumglass/al_master_data/doctype/al_pricing_dimension/al_pricing_dimension.json`

Trong mảng `fields`, thêm object sau (có thể đặt cạnh `is_required_for_price` hoặc `is_active`):
...
**File mới:** `alumglass/patches/v28_12/set_represents_color_flag.py`
...
**Sau đó thêm dòng này vào `alumglass/patches.txt`** (trong khối `[post_model_sync]`, sau `v28_11.rebackfill_cost_template_pre_vat_price`):

```
alumglass.patches.v28_12.set_represents_color_flag
```

### D.3 – Sửa `bom_orchestrator.py` – hàm `_color_variable_for_category` và thêm `_get_color_dimension_codes`

**Thay thế toàn bộ hàm `_color_variable_for_category` (khoảng dòng 1295–1309) bằng:**
...
### E.1 – Tạo module `fb_config.py`

**File mới:** `alumglass/al_bom_engine/fb_config.py`
...
### E.2 – Sửa `install_fb_bindings.py`

**File:** `alumglass/setup/install_fb_bindings.py` – thêm vào cuối:
...
DEFAULT_ACCESSORY_CATEGORY_BINDING = {
    ...
    "source_config": {
        ...
        "value": "PK",   # ← ĐỔI THÀNH TÊN THẬT TRƯỚC KHI DEPLOY
    },
    "applies_to_doctype": "",
    "is_global": 1,
    "is_active": 1,
...
**File:** `alumglass/hooks.py` – trong cả hai hàm `_after_install` và `_after_migrate`, thêm dòng import và gọi `install_named_constant_bindings`:
...
```python
    ...
    install_named_constant_bindings()   # ← THÊM
```

### E.4 – Sửa `bom_orchestrator.py` – hàm `_load_accessories`

Tìm dòng gán `"color_group_category": "PHU_KIEN"` và sửa thành:
...
### E.5 – Sửa `api/__init__.py` (dòng ~805-813)
...
**File:** `formula_builder/api/data_source_registry.py` – chèn toàn bộ code dưới đây **sau khối `composite_key_lookup`** (tìm dòng `# ═══════════════════════════════════════════════════════════════════════════` để xác định vị trí).
...
## Phần G – Mở rộng `linked_doctype_field` (auto-discover)

**File:** `formula_builder/api/data_source_registry.py` – sửa hàm `_handle_linked_doctype_field` và `_resolve_linked_doctype_field_batch`.

**Thay thế toàn bộ hàm hiện tại (dòng ~279-307) bằng:**
...
**Sau đó, trong hàm `_resolve_linked_doctype_field_batch` (dòng ~310-345), thêm logic tương tự.** Thay thế toàn bộ hàm đó bằng:

```python
def _resolve_linked_doctype_field_batch(bindings, doc, resolved_so_far):
    results = {}
    if not doc:
        return results
    # Gom nhóm theo fingerprint (đã có sẵn, nhưng ta có thể dùng logic đơn giản)
    # Để đơn giản, xử lý từng binding nhưng vẫn tận dụng cache của frappe.get_meta
    for b in bindings:
        results[b["variable_name"]] = _handle_linked_doctype_field(b, doc, resolved_so_far)
    return results
...
## Phần H – `child_table_lookup` – source type mới

**File:** `formula_builder/api/data_source_registry.py` – chèn sau `range_lookup` (hoặc cuối file).
...
# II. SỬA THEO BẢN REVIEW TOÀN DIỆN (bổ sung)

## A.1 – Bật workflow

**File:** `alumglass/hooks.py` – trong `_after_install` và `_after_migrate`, bỏ comment dòng `install_workflows()`:

```python
    from alumglass.setup.install_workflows import install_workflows   # thêm nếu chưa có
    install_workflows()
```

**Lưu ý:** Trước khi bật, kiểm tra rằng các role `AL BOM Manager`, `AL Technical Admin`, `AL Site Engineer` đã tồn tại và có người được gán.

---

## A.2 – Ẩn AL Alert Config và AI Suggestion Log (nếu chưa có evaluator)

**Cách 1 (nhanh):** Đặt `hidden=1` trong doctype JSON của `AL Alert Config` và `AL AI Suggestion Log`.

Trong file `alumglass/al_ai_intelligence/doctype/al_alert_config/al_alert_config.json`, thêm `"hidden": 1` vào cấp cao nhất.

Tương tự cho `al_ai_suggestion_log.json`.

**Cách 2 (nếu muốn giữ UI nhưng chặn tạo mới):** Sửa permissions (trong `al_alert_config.json` → `permissions` → thêm role `System Manager` chỉ có quyền `read`, không ai có `write`). Tạm thời tôi khuyến nghị cách 1.

---

## A.3 – Xóa dead code `custom_field_quotation.py`

Xóa toàn bộ thư mục: `alumglass/al_selling/doctype/custom_fields/` (hoặc ít nhất là file `custom_field_quotation.py`).

```bash
rm -rf alumglass/al_selling/doctype/custom_fields/
```

---

## A.4 – Thêm `read_only=1` cho các field tính toán

### Site Survey Item

**File:** `alumglass/al_construction/doctype/al_site_survey_item/al_site_survey_item.json`

Trong mảng `fields`, với các field `width_deviation_mm`, `height_deviation_mm`, `within_tolerance`, thêm thuộc tính `"read_only": 1`.

Ví dụ:

```json
{
    "fieldname": "width_deviation_mm",
    "fieldtype": "Float",
    "label": "Width Deviation (mm)",
    "read_only": 1
}
```

### Glass Cutting Item (nếu có field `offcut_reusable` tính tự động)

**File:** `alumglass/al_manufacturing/doctype/al_glass_cutting_item/al_glass_cutting_item.json` – tương tự, thêm `read_only=1` cho `offcut_reusable`.

---

## A.5 – Phân quyền cho checkbox phê duyệt Change Order

**File:** `alumglass/al_account/doctype/al_change_order/al_change_order.json`

Trong mảng `fields`, tìm `customer_approval` và `internal_approval`, thêm `"permlevel": 1` vào cả hai.
...
Sau đó, trong `al_change_order.json` → `permissions`, cần đảm bảo role `AL Project Accountant` hoặc `AL Technical Admin` có quyền `write` ở `permlevel=1`. Bạn có thể thêm dòng:

```json
{
    "role": "AL Project Accountant",
    "read": 1,
    "write": 1,
    "permlevel": 1
}
...
## B.1 – Thêm `is_active` cho AL Variable Dimension Mapping và AL Slug Library

### AL Variable Dimension Mapping

**File JSON:** `alumglass/al_master_data/doctype/al_variable_dimension_mapping/al_variable_dimension_mapping.json` – thêm field:
...
Sau đó, trong tất cả các query đọc mapping (đã đề cập trong BAN-FULL-A-Z Phần B/D), thêm filter `is_active=1`. Cụ thể:

- Trong `_get_dim_mappings_raw()` của `bom_orchestrator.py`, sửa:

```python
mappings = frappe.get_all(
    "AL Variable Dimension Mapping",
    filters={"is_active": 1},   # ← thêm
    fields=["name", ...]
)
```
...
**File JSON:** `alumglass/al_master_data/doctype/al_slug_library/al_slug_library.json` – tương tự thêm `is_active` với default 1.

Khi tạo `AL Bom Item` hoặc `AL Accessory Item`, dropdown `slug` nên lọc active slug. Trong `al_bom_item.py` (validate), bạn có thể thêm kiểm tra.

---

## B.1 – Thêm `weight_kg_per_m2` và `max_area_m2` cho AL Glass Master

**File JSON:** `alumglass/al_master_data/doctype/al_glass_master/al_glass_master.json` – thêm vào mảng `fields`:

```json
{
    "fieldname": "weight_kg_per_m2",
    "fieldtype": "Float",
    "label": "Weight (kg/m²)",
    "description": "Trọng lượng trên m² kính, dùng cho tính tải trọng và vận chuyển"
},
{
    "fieldname": "max_area_m2",
    "fieldtype": "Float",
    "label": "Max Area (m²)",
    "description": "Diện tích tối đa cho phép cho 1 tấm kính này (theo tiêu chuẩn an toàn)"
}
```

---

## B.6 – Thêm `assigned_to` cho Punchlist và `company_representative_signature` cho Handover

### AL Punchlist Item

**File JSON:** `alumglass/al_construction/doctype/al_punchlist_item/al_punchlist_item.json` – thêm:
...
```json
{
    ...
    "label": "Assigned To",
    "options": "Employee",
    "description": "Người chịu trách nhiệm xử lý hạng mục này"
}
```

### AL Handover Acceptance

Thêm `company_representative_signature`:

```json
{
    "fieldname": "company_representative_signature",
...
## B.4 – Thêm `previous_unit_price` cho AL Supplier Price List Item

**File JSON:** `alumglass/al_buying/doctype/al_supplier_price_list_item/al_supplier_price_list_item.json` – thêm:

```json
{
    "fieldname": "previous_unit_price",
    "fieldtype": "Currency",
    "label": "Previous Unit Price",
    "read_only": 1,
    "description": "Giá kỳ trước (tự động so sánh)"
}
```

Sau đó, trong `al_supplier_price_list_item.py` (hoặc `on_update` của parent), bạn cần tính toán `previous_unit_price` bằng cách truy xuất bản ghi cũ cùng item_code và supplier. (Tôi có thể cung cấp code nếu cần, nhưng phạm vi bài này chưa yêu cầu).

---

## C.2 – Bảo hành theo cấu kiện – thêm child table

### Tạo doctype mới `AL Warranty Coverage Row` (child table)

**File JSON:** `alumglass/al_master_data/doctype/al_warranty_coverage_row/al_warranty_coverage_row.json`

```json
{
    "doctype": "DocType",
    "name": "AL Warranty Coverage Row",
    "istable": 1,
    "fields": [
        {
            "fieldname": "defect_category",
            "fieldtype": "Select",
        ...
        {
            "fieldname": "coverage_condition",
            "fieldtype": "Small Text",
            "label": "Coverage Conditions/Exclusions"
        }
    ]
}
```

Sau đó, thêm child table vào `AL Warranty Policy`:

**File:** `alumglass/al_master_data/doctype/al_warranty_policy/al_warranty_policy.json` – thêm field:

```json
{
    "fieldname": "coverage_rows",
    "fieldtype": "Table",
    "label": "Coverage by Defect Category",
    "options": "AL Warranty Coverage Row"
}
```

Trong `warranty_claim.py` (hoặc bất cứ nơi nào tính thời hạn bảo hành), ưu tiên tra `coverage_rows` theo `defect_category`, nếu không có thì fallback vào `warranty_months` header.

---

## C.4 – Thêm status và approval cho AL Material Plan

**File JSON:** `alumglass/al_buying/doctype/al_material_plan/al_material_plan.json` – thêm:
...
Bạn có thể thêm logic trong `on_update` để tự động set `approved_by` khi status chuyển sang `Reviewed` (bằng cách kiểm tra người dùng hiện tại). Nếu muốn đơn giản, có thể để nhập tay.

---

## C.3 – Tạo doctype `AL Installation Team Booking` (đề xuất)

**File JSON mới:** `alumglass/al_construction/doctype/al_installation_team_booking/al_installation_team_booking.json`

```json
{
    "doctype": "DocType",
    "name": "AL Installation Team Booking",
    "autoname": "field:installation_order",
    "fields": [
        {
            "fieldname": "installation_team",
            "fieldtype": "Link",
            "label": "Installation Team",
            "options": "AL Installation Team",
            "reqd": 1
        },
        {
...
Sau đó, trong `AL Installation Order` (khi lưu), bạn có thể tạo/xóa/cập nhật các booking tương ứng. Mã có thể được viết trong `al_installation_order.py` (hàm `on_update`).

---

## Bổ sung – Validate `AL Color Standard` chặn nhiều is_default

**File:** `alumglass/al_master_data/doctype/al_color_standard/al_color_standard.py`
...
def validate(self):
    if self.is_default:
        # Kiểm tra xem đã có màu khác cùng applies_to đang là default chưa
        existing = frappe.db.get_value(
            "AL Color Standard",
            {"applies_to": self.applies_to, "is_default": 1, "name": ["!=", self.name]},
            "name"
        )
        if existing:
            frappe.throw(f"Màu '{existing}' đã được đặt là default cho nhóm {self.applies_to}. Chỉ được có 1 default.")
```
...
Trên đây là toàn bộ code sửa lỗi chi tiết cho cả hai tài liệu. Bạn có thể copy-paste từng phần vào đúng file và vị trí đã chỉ dẫn.

**Lưu ý quan trọng:**
- Trước khi triển khai, hãy sao lưu database và code.
- Chạy `bench migrate` sau khi thêm field JSON và patch.
- Kiểm tra kỹ các role và người dùng trước khi bật workflow.
- Với những thay đổi ở `formula_builder`, chạy đầy đủ test suite trước khi merge.
