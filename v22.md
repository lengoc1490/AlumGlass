...
## XV. BOMORCHESTRATOR — QUY TRÌNH 9 BƯỚC (v19, cập nhật ★v23 QĐ-26)

**★v23 — Nguyên tắc #29, QĐ-26:** BomOrchestrator.resolve_version() xác định version BOM/Rule dựa trên `Quotation.creation_date` so với `valid_from` — không đọc `current_version` sống:

```
resolve_version(doc_creation_date, version_list):
  1. Lọc các version có valid_from <= doc_creation_date
  2. Chọn version có valid_from lớn nhất (version mới nhất tại thời điểm tạo)
  3. Nếu current_version.valid_from > doc_creation_date:
       → Badge "Đang dùng BOM v{cũ} (đã có v{mới} từ {valid_from})"
       → Nút "Cập nhật lên BOM mới nhất" cho phép Sales chủ động nâng version
  4. Kết quả resolve được ghi vào ConfigSnapshot — bất biến sau Submit
```

Pattern này tái dùng chính xác `valid_from`/`valid_to` của ERPNext Item Price — đơn giản, deterministic, audit được, không phụ thuộc cache TTL.

| Bước | Component | Chi tiết |
|---|---|---|
| **0 ★v23** | **VersionResolver** | **Pin version BOM + toàn bộ Rule theo `Quotation.creation_date` so với `valid_from`. Kết quả lưu vào context, dùng cho toàn bộ các bước sau** |
| 1 | VariableResolver.resolve() | `inputs_dict` đầy đủ + `glass_thick_{prefix}` |
| 2 ★v18 | DynamicItemResolver.resolve() | Pre-resolve item cho dòng Dynamic → inject `{slug}_item_code`, `{slug}_kg_per_m`, `{slug}_price` vào `inputs_dict`. ★v23: Rule version đã được pin từ bước 0 — không đọc `current_version` sống |
...
| 7 | **FormulaEngine.calculate() + SnapshotBuilder.persist()** | **DUY NHẤT 1 lần calculate()** — ghi `ConfigSnapshot` (kèm `snapshot_version` ★v23) |
...
`bom` (Link), `version_number` (Int), `status` (Select), `published_on/by`, `profile_set_snapshot` (JSON), `variable_set_snapshot` (JSON), `pk_set_snapshot` (JSON), `cost_template_snapshot` (JSON), `snapshot_hash` (Data), **`snapshot_version` (Int, ★v23 — schema version cho Schema Registry)**, **`valid_from` (Datetime, ★v23 QĐ-26 — mặc định = `published_on`, BomOrchestrator resolve version theo `Quotation.creation_date` so với trường này)**, **`valid_to` (Datetime, ★v23 — tự set khi version mới Published, version hiện hành để trống)**, `change_summary`, `approved_by`, `is_rollback_of` (Link, self), `al_change_logs` (Table).
...
**★v23 — Khóa field kích thước trên SO (P0-2):** field `al_W_mm`/`al_H_mm`/`al_bom_vars` trên SO Item → **read-only** sau khi SO Submit. Mọi thay đổi kích thước phải đi qua luồng: Site Survey → BOM Revision → ConfigSnapshot mới → Change Order. Validate ở SO `before_save`: nếu `al_config_snapshot` đã có và field thay đổi → reject. Đây là hệ quả trực tiếp của Nguyên tắc #26 — chặn "cửa sau" sửa tay kích thước trên SO mà không qua Site Survey.

---

### 25.8 ★v23 — MATERIAL CHECK HOOK: KIỂM TRA VẬT TƯ TRƯỚC KHI CONFIRM CUTTING PLAN (Nguyên tắc #28)

**Bối cảnh:** Cutting Plan Confirm khi batch chưa về kho → cắt xong không có hàng ráp → bán thành phẩm nằm chờ → trễ tiến độ. Với cơ chế `is_standard_stock=0` (màu đặt riêng) + `deposit_required_pct` (§20.3), một dòng Cutting Plan có thể Confirm trong khi batch chưa về kho (deposit chưa paid, NCC chưa giao). Hậu quả không thể đảo ngược: phiếu cắt đã in, công nhân đã setup máy.

**Vị trí hook:** đăng ký ở Tầng 6C, chạy **trước** `AL Aluminum/Glass Cutting Plan.status Draft → Confirmed`. Hard block — không phải soft warning.

**3 điều kiện bắt buộc:**

| Điều kiện | Kiểm tra | Hành động nếu fail |
|---|---|---|
| (a) Batch đã có hàng trong kho | Với mỗi Cutting Plan Line có `batch_no` cố định → query Stock Ledger `qty_after_transaction` tại warehouse liên quan | **Hard block** — hiển thị: "Batch {batch_no} không đủ số lượng trong kho (cần {qty}, có {available})" |
| (b) Deposit đã paid cho màu đặt riêng | Với dòng `is_standard_stock=0` → kiểm tra `AL Material Plan Line.deposit_status=Paid` | **Hard block** — hiển thị: "Màu {color_code} chưa đặt cọc NCC — không thể cắt khi chưa đảm bảo nguồn cung" |
| (c) PO/Subcontracting Order đã đặt cho dòng gia công | Với dòng `is_outsourced=1` → kiểm tra PO/Subcontracting Order tương ứng đã Submit | **Hard block** — hiển thị: "Dòng {line} chưa có PO/Subcontracting Order — không thể cắt khi chưa đặt gia công ngoài" |

**Cấu hình theo dự án:** `AL Project Financial Config` có field `enforce_material_check` (Check, mặc định bật). Nếu tắt → soft warning thay vì hard block (chỉ dùng cho dự án nội bộ/trưng bày không ảnh hưởng tiến độ cam kết khách hàng).

### 25.9 ★v23 — AL QUALITY CHECK: GATE TRƯỚC INSTALLATION

**Bối cảnh:** `AL Production Order Bridge.production_stage` đã có giá trị "QC" nhưng chưa có DocType ghi nhận kết quả. Cần một gate cứng: `production_stage` không thể chuyển "Ready to Install" nếu chưa có QC Passed.

**AL Quality Check (Master, #70):**

| fieldname | fieldtype | reqd | Mô tả |
|---|---|---|---|
| production_order_bridge | Link → AL Production Order Bridge | ✓ | |
| check_date | Date | ✓ | |
| checked_by | Link → Employee | ✓ | |
| result | Select | ✓ | Passed / Failed / Rework |
| defect_notes | Small Text | | Mô tả lỗi nếu Failed/Rework |
| photos | Attach Image | | Ảnh lỗi (tùy chọn) |

**Quy tắc gate:** Hook trên `AL Installation Order` — không thể chuyển `In Progress` nếu `production_stage != "Ready to Install"` (tức QC chưa Passed cho tất cả Production Order Bridge liên quan). Đây là lớp đảm bảo chất lượng vật lý: sản phẩm phải đạt QC tại xưởng trước khi được chở đi công trình lắp đặt.
...
| **new_bom_version** | **Link → AL BOM Version** | **★v23 (P1-4).** Bắt buộc nếu `change_type=ADD_ITEM` và `bom_reference` có giá trị — tham chiếu chính xác version BOM mới đã publish, làm chặt liên kết lỏng lẻo (`bom_reference` optional cũ). Không dùng để thay thế cơ chế versioning kỹ thuật — chỉ là lớp kiểm soát giá trị hợp đồng |
| description | Small Text | |
| qty | Float | |
| unit_price | Currency | |
...
`milestone_name` (vd "Tạm ứng 30%", "Hoàn thành 70%", "Nghiệm thu bàn giao"), `billing_pct`, `trigger_type` (MANUAL / INSTALLATION_PROGRESS_PCT / DATE / CUSTOMER_ACCEPTANCE ★v22 / **INSTALLED_M2 ★v23 (P1-5)** — nhiều hợp đồng thanh toán theo m² lắp đặt thực tế thay vì %), `trigger_value` (với `INSTALLED_M2`: số m² lắp đặt thực tế cần đạt; với `INSTALLATION_PROGRESS_PCT`: % tiến độ; với `DATE`: ngày; với `CUSTOMER_ACCEPTANCE`/`MANUAL`: bỏ trống), `status` (Not Ready/Ready to Bill/Invoiced/Paid), `sales_invoice` (Link).

### 27.4 Luồng
| Bước | Actor | Hành động |
|---|---|---|
| 1 | (Hook, sau khi `AL Installation Progress` hoặc `AL Handover Acceptance` save) | Với `trigger_type=INSTALLATION_PROGRESS_PCT`: kiểm tra `cumulative_pct` vs `trigger_value`. Với `trigger_type=CUSTOMER_ACCEPTANCE` (★v22): kiểm tra `AL Handover Acceptance.status=Signed` tương ứng. Với `trigger_type=INSTALLED_M2` (★v23): kiểm tra tổng m² đã lắp từ `AL Installation Progress` — **ba điều kiện độc lập, không dùng % tiến độ nội bộ thay cho chữ ký khách hàng hoặc khối lượng thực tế** |
...
### 28.7 ★v23 — PROJECT/COST CENTER ENFORCEMENT HOOKS (P1-2)

**Bối cảnh:** ERPNext đã có sẵn `project` và `cost_center` trên Purchase Invoice, Stock Entry, Journal Entry — không cần custom field nào. Cái thiếu là cơ chế tự động populate và validate cứng, đảm bảo `material_cost_actual` trong `AL Project Profitability Snapshot` khớp với sổ cái.

**Hook tự động populate (đăng ký ở Tầng 6E):**

| Transaction | Hook | Hành động |
|---|---|---|
| Purchase Invoice | `before_save` | Nếu PI item ← PO ← Material Plan ← SO → tự động set `Purchase Invoice Item.project` và `cost_center` từ `AL Project Financial Config` |
| Stock Entry | `before_save` | Nếu SE Detail ← Cut Order ← SO → tự động populate `project` trên Stock Entry Detail |
| Journal Entry | `before_save` | Nếu JE Account liên quan đến Project có `AL Project Financial Config` → tự động set `project` |

**Validate cứng (hard block):**

| Transaction | Điều kiện | Hành động |
|---|---|---|
| PI/SE/JE Submit | Item/tham chiếu thuộc dự án có `AL Project Financial Config` | `project` không được để trống → reject với thông báo rõ |
| PI/SE Submit | `project` khác với Project được map từ SO | Reject — "Giao dịch này thuộc dự án {expected}, không thể gán vào {actual}" |

**Triển khai:** đây là hook tiêu chuẩn, không cần custom field, không cần DocType mới. Chỉ cần validate logic trong `before_submit` + tự động populate trong `before_save` — đúng tinh thần Nguyên tắc #1 và #13.

### 28.8 ★v23 — FIELD-LEVEL PERMISSION CHO DỮ LIỆU GIÁ VỐN (P1-6)

Frappe hỗ trợ `permlevel` trên field + Role Permission Manager sẵn có. Sales chỉ thấy giá bán, không thấy giá vốn.

| DocType.Field | permlevel | Vai trò được đọc |
|---|---|---|
| `AL Cost Template Line.actual_cost` | 1 | Admin, Kế toán, Director |
| `AL Project Profitability Snapshot.*` | 1 | Admin, Kế toán, Director |
| `AL Cost Variance.variance_pct` | 1 | Admin, Kế toán, Director, Mua Hàng (chỉ item mình phụ trách) |
| `AL Labor Cost Variance.variance_pct` | 1 | Admin, Kế toán, Director, Quản lý Thi công |

Sales và Đội Thi Công mặc định không có quyền đọc field permlevel ≥ 1. Triển khai qua Role Permission Manager (ERPNext core, không cần code Python).
...
|---|---|---|
| **Phase 0 ★v20 — chèn TRƯỚC mọi phase khác** | **Sửa lỗi kiến trúc tồn kho màu (§4.23)**: tạo `AL Color Standard`, bật Batch cho Item nhôm/vật tư áp dụng, migration nếu đã lỡ tạo Item Variant. **Verify R27 (★v23 — 2 lớp: vật lý vs kế toán):** test Batch Wise Valuation trên đúng version ERPNext đang dùng cho ≥1 Item có ≥2 màu — phân biệt kết quả theo 2 lớp, không "được ăn cả, ngã về không" | Không còn Item Variant theo màu nào tồn tại; MRP Lite chạy thử với dữ liệu Batch cho ≥1 dự án thí điểm khớp thủ công; **báo cáo Gross Profit theo batch đối chiếu; R27 đánh giá theo 2 lớp** |
| **1 — Core Engine** | Tầng 1-4: Master Data, Rule/Formula Engine, Orchestration 9 bước gốc. **★v23 đồng thời:** `valid_from`/`valid_to` trên BOM/Rule Version + BomOrchestrator.resolve_version() theo QĐ-26; `snapshot_version` + Schema Registry nền tảng; **P0-2: khóa field `al_W_mm`/`al_H_mm` trên SO** | Ví dụ 1-5 + A-D PASS; không `eval()`; `explain()` đúng; version resolve theo `creation_date` đúng; SO từ chối sửa tay kích thước |
| **2 — Module Commercial + Vá lỗ hổng liên module ★v23** | BOM Version, Approval, Discount, Reporting cơ bản, MRP Lite (theo Batch-màu, đặt cọc NCC ★v22 §20.3), Cost Variance, Notification, Sales Analytics, `AL Supplier Price List` + cảnh báo `SUPPLIER_PRICE_CHANGE` (★v22 §4.24). **★v23 đồng thời:** Custom field `al_trace_stage` trên Batch (Material Trace P1-7); hook tự động populate `project`/`cost_center` trên PI/SE (P1-2); field-level permission giá vốn (P1-6) | Ví dụ E-G PASS; Material Trace tự động ghi nhận ISSUED/CUT/INSTALLED; PI/SE không missing project |
| **2.5 — Vá Rule Versioning (P0, bắt buộc trước Phase 3)** | §VIII đầy đủ: AL Dynamic Item Rule Version + luồng publish/approve/diff + **★v23 valid_from/valid_to** | Ví dụ H PASS; migration tạo version v1 cho mọi rule hiện có |
| **3 — Sản Xuất** | Cutting Standard, Aluminum/Glass Cutting Plan (gia công/thuê ngoài ★v22), Cut Order (đã cập nhật theo §4.23). **★v22: `AL Site Survey` — cổng bắt buộc trước Cutting Plan (§25.7, Nguyên tắc #26). ★v23 đồng thời: Material Check Hook (P0-1, §25.8 — hard block trước Confirm); AL Quality Check gate (P1-3, §25.9)** | Ví dụ I PASS; phế liệu giảm so với baseline; không phát sinh Item mới; 100% Cutting Plan Line có Site Survey liên kết trước Confirm; **không Cutting Plan nào Confirm khi chưa đủ vật tư; không Installation Order nào In Progress khi chưa QC Passed** |
| **4 — Thi Công + Thanh Quyết Toán** | Installation Team/Order/Progress/Cost, Warranty, Milestone Billing. **★v22: `AL Change Order` (§27.0, Nguyên tắc #27), `AL Handover Acceptance` (§27.5). ★v23 đồng thời: `new_bom_version` trên Change Order Line (P1-4), `trigger_type=INSTALLED_M2` trên Milestone Billing Line (P1-5)** | Ví dụ J PASS; mobile app hoạt động; không có sửa tay `total_contract_value` ngoài luồng Change Order; INSTALLED_M2 trigger hoạt động |
...
| **6 — AI P0-P1 + vận hành nền tảng ★v21** | Cutting Optimization, Project Health Score, BOM/Rule AI-assist, Quotation Copilot; **đồng thời triển khai QĐ-21→27**: ConfigSnapshot compressed, Sales Override control, AI Hooks Framework, CostAccumulator v2 fallback logging, DynamicItemResolver v2, Version resolve valid_from, Material Check Hook | Mọi suggestion qua `AL AI Suggestion Log`/`AL AI Interaction Log` + Approval Workflow; không có đường tắt |
...
> **★v23 bổ sung:** Các lỗ hổng P0 (Material Check Hook, khóa SO field, verify R27) phải được xử lý **trong Phase 1-3** — không thể để đến Phase 5+ vì đây là các lỗ hổng gây thiệt hại tài chính thực tế. `valid_from`/`valid_to` và `snapshot_version` nên triển khai ngay từ Phase 1 để tránh migration về sau.

---

## XXXV. RỦI RO & ĐIỂM THEO DÕI (HỢP NHẤT TOÀN BỘ, 33 RỦI RO — v23)
...
| R27 ★v22 (sửa ★v23) | "Batch Wise Valuation" của ERPNext không tách giá vốn tuyệt đối theo batch trong mọi report | **Cao — phải verify trước Phase 0, nhưng phân biệt 2 lớp:** (a) **Mục tiêu vật lý** (không lắp nhầm màu — quan trọng nhất, Batch đạt 100%): nếu verify chỉ đạt (a) → hệ thống vẫn có giá trị lớn. (b) **Mục tiêu kế toán** (giá vốn chính xác tuyệt đối — quan trọng nhưng ít nghiêm trọng hơn nếu sai lệch nhỏ): không cần đại tu sang giải pháp khác nếu chưa hoàn hảo | Test đối chiếu Gross Profit theo batch cho ≥1 Item có ≥2 màu trên đúng version ERPNext đang dùng; phân biệt kết quả theo 2 lớp (a) và (b) — không "được ăn cả, ngã về không" |
...
| R30 ★v22 | Khách hàng từ chối ký `AL Handover Acceptance` (status=Disputed) làm kẹt milestone billing dây chuyền | Med | Milestone billing dòng `CUSTOMER_ACCEPTANCE` không chặn các dòng `MANUAL`/`DATE`/`INSTALLED_M2` khác trong cùng plan; Director có dashboard riêng theo dõi Disputed để can thiệp thương lượng sớm |
...
| **R32 ★v23** | **Đội Sản xuất bỏ qua Material Check Hook (tắt `enforce_material_check` cho dự án quan trọng)** | **Cao nếu sai** | `enforce_material_check` mặc định bật, chỉ Director được tắt; log audit mọi lần tắt; dashboard Production hiển thị danh sách Cutting Plan đã Confirm khi chưa đủ vật tư |
| **R33 ★v23** | **QC gate bị bỏ qua do `AL Production Order Bridge` không được dùng (công ty nhỏ không dùng Work Order)** | **TB** | QC gate là tùy chọn: nếu không dùng Production Order Bridge → Installation Order không bị chặn; khuyến nghị ít nhất dùng AL Quality Check thủ công cho công ty nhỏ |
...
- [ ] **★v23 Material Check Hook đã test cho ≥3 kịch bản: (a) đủ hàng → pass, (b) thiếu batch → hard block, (c) deposit chưa paid → hard block; `enforce_material_check` hoạt động đúng**
- [ ] **★v23 Field `al_W_mm`/`al_H_mm` trên SO đã khóa read-only sau Submit; test thử sửa tay → reject**
- [ ] **★v23 `valid_from`/`valid_to` trên AL BOM Version và AL Dynamic Item Rule Version hoạt động đúng: Quotation dùng version tại thời điểm creation_date; badge "Có bản mới" hiển thị khi current version mới hơn**
- [ ] **★v23 `snapshot_version` + Schema Registry đã có migration function cho snapshot schema hiện tại; roundtrip test pass**
- [ ] **★v23 Hook tự động populate `project`/`cost_center` trên PI/SE từ SO đã test — không có giao dịch nào bị missing project**
- [ ] **★v23 Batch custom field `al_trace_stage` hoạt động: Stock Entry → ISSUED, Cutting Plan confirm → CUT, Installation done → INSTALLED**
- [ ] **★v23 AL Quality Check gate: không thể chuyển Installation Order In Progress nếu production_stage != Ready to Install**
- [ ] **★v23 Field-level permission (permlevel) cho dữ liệu giá vốn đã cấu hình; Sales không thấy được AL Cost Template Line.actual_cost và AL Project Profitability Snapshot**
- [ ] **★v23 `trigger_type=INSTALLED_M2` trên Milestone Billing Line hoạt động: khi tổng m² lắp đạt trigger_value → Ready to Bill**
- [ ] **★v23 `new_bom_version` trên AL Change Order Line validate: bắt buộc nếu change_type=ADD_ITEM và bom_reference có giá trị**

---

*AlumGlass ERP — Đặc Tả Hợp Nhất (Canonical) · Kế thừa toàn bộ v11 → v13 → v14 → v15 → v16 → v17 → v18 → v19 → v20 → v21 → v22 → v23*
...
*"Khai báo theo thứ tự tư duy nghề. Tính theo thứ tự DAG. Mở rộng theo module không phá lõi. Tồn kho theo thực tế vật lý (Batch). Version có hiệu lực theo thời gian (valid_from). Kích thước theo thực tế công trình (Site Survey). Không cắt khi chưa có hàng (Material Check). Phạm vi hợp đồng thay đổi có kiểm soát (Change Order). Minh bạch từ báo giá tới quyết toán. AI luôn đề xuất, không quyết định."*
