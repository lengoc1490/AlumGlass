...
| 22 | **Màu (nhôm/phụ kiện) là thuộc tính GIAO DỊCH — quản lý qua Inventory Dimension (mặc định) hoặc Batch (phương án đơn giản hoá), KHÔNG BAO GIỜ là trục biến thể (Attribute) của Item Master** | Tránh nổ Item ảo — xem §IV.23. Phương án mặc định: Hybrid (Inventory Dimension + Batch). Batch-màu thuần tuý là phương án cho công ty nhỏ (<20 màu) |
...
**Nguyên tắc:** tồn kho nhôm/vật tư theo màu được quản lý bằng **một trong hai phương án**, tuỳ quy mô doanh nghiệp. Cả hai đều tránh Item Variant.

| Phương án | Mô tả | Khi nào chọn |
|---|---|---|
| **A. Hybrid (mặc định)** | Inventory Dimension cho màu sắc + vị trí kho, Batch chỉ để track lô và al_trace_stage, Batch Wise Valuation TẮT | Doanh nghiệp vừa/lớn, ≥20 màu, cần quản lý vị trí kho, cần COGS ổn định |
| **B. Batch-màu (đơn giản hoá)** | Batch mang màu + Batch Wise Valuation BẬT, không cần Inventory Dimension | Doanh nghiệp nhỏ, <20 màu, không cần vị trí kho, muốn setup nhanh |

> **Chi tiết từng phương án xem bên dưới.** Phương án A (Hybrid) được thiết kế đầy đủ ở §4.23.7. Phương án B (Batch-màu) được thiết kế ở §4.23.2–4.23.6.
...
### 4.23.7 ★ Phương án A — Hybrid (Inventory Dimension + Batch) — MẶC ĐỊNH

> **Đây là phương án khuyến nghị cho đa số doanh nghiệp.** Kết hợp Inventory Dimension core (cho màu sắc, vị trí kho) với Batch tracking (cho lô, trace_stage). Phương án B (Batch-màu, §4.23.2–4.23.6) vẫn được support đầy đủ cho doanh nghiệp nhỏ.
>
> **Quyết định chọn Hybrid làm mặc định dựa trên:** (a) Query tồn kho theo màu trực tiếp không join Batch; (b) Pool valuation giúp COGS ổn định khi giá nhôm biến động; (c) Quản lý được vị trí kho; (d) Batch giải phóng khỏi nhiệm vụ lưu màu → chỉ làm tracking đúng bản chất; (e) Dễ thêm dimension mới (độ dày, bề mặt) sau này không cần code.

#### 4.23.7.1 Vì sao cần phương án bổ sung?

Batch-màu (§4.23.2) giải quyết tốt vấn đề **tránh bùng nổ Item Variant**, nhưng có một số hạn chế khi vận hành ở quy mô lớn hoặc yêu cầu quản lý đa chiều:

| Hạn chế | Mô tả | Ảnh hưởng |
|---|---|---|
| **Query tồn kho theo màu** | Phải join Batch để lọc `al_color` — không filter trực tiếp trên SLE | Chậm, phức tạp khi viết báo cáo |
| **Batch Wise Valuation** | Mỗi lô màu có valuation riêng → cùng item, cùng màu nhưng COGS khác nhau nếu mua 2 đợt giá khác nhau (VD: Trắng LOT-001 giá 10.000, LOT-002 giá 12.000) | Kế toán biến động, khó giải trình |
| **Vị trí kho (kệ)** | `al_bin_location` (Data text) chỉ để tra cứu thủ công — không thể query hay báo cáo | Không kiểm kê được theo vị trí |
| **Thêm thuộc tính mới** (VD: độ dày, bề mặt) | Phải thêm field vào Batch hoặc tạo Item mới | Kém linh hoạt |
| **Stock Reconciliation định kỳ** | Core chặn SR có `batch_no` — phải custom | Thêm chi phí bảo trì |

#### 4.23.7.2 Giới thiệu Inventory Dimension — cơ chế core ERPNext

ERPNext có cơ chế **Inventory Dimension** (từ v14+), cho phép thêm custom fields vào Stock Ledger Entry một cách tự động, mà không cần tạo Item Variant hay Batch mới.

**Cách hoạt động:**
1. Admin tạo Inventory Dimension record (VD: "Màu sắc", "Vị trí kệ")
2. Core tự động thêm field tương ứng vào: Stock Entry Detail, Stock Reconciliation Item, Purchase Receipt Item, Delivery Note Item, **Stock Ledger Entry**
3. Khi nhập/xuất, SLE tự động ghi giá trị dimension
4. Query tồn kho: `get_stock_balance(item_code, warehouse, inventory_dimensions_dict={"mau_sac": "Trắng"})`
5. Valuation engine (AVCO/FIFO) vẫn dùng **pool chung warehouse-level** — chỉ qty phân theo dimension

**Chi tiết kỹ thuật (xác nhận từ code thật v14.92.14):**
```python
# stock_ledger.py — process_sle() keyed bằng WAREHOUSE, không phải (warehouse + dimensions)
...
# get_stock_balance với dimension_dict filter → qty tại dimension đó, rate pool chung
→ Số lượng phân theo dimension, giá trị pool chung warehouse
```

**Đây chính là mô hình chuẩn của SAP (Plant-level valuation, Bin chỉ track qty), Oracle NetSuite (Location-level, Bin không ảnh hưởng cost), Dynamics 365 (Financial/Physical dimension) — xem phân tích đầy đủ tại tài liệu ERPNext_Stock_Reconciliation_Inventory_Dimension_Giai_Phap_v5.md §20.**

#### 4.23.7.3 So sánh Batch-màu (hiện tại) vs Inventory Dimension (bổ sung)

| Tiêu chí | Batch-màu (mặc định) | Inventory Dimension (bổ sung) |
|---|---|---|
| **Màu sắc** | `Batch.al_color` | `SLE.mau_sac` (field trên SLE, core tự thêm) |
| **Vị trí kệ** | `al_bin_location` (Data text) | `SLE.vi_tri_kho` (Link → Vi tri kho / Location) |
| **Batch Wise Valuation** | ✅ Bắt buộc (mỗi lô màu 1 rate riêng) | ❌ Tắt — pool AVCO/FIFO chung warehouse |
| **Query tồn theo màu** | Join Batch → filter `al_color` | Direct: `inventory_dimensions_dict={"mau_sac": "Trắng"}` |
| **Query tồn theo vị trí kệ** | Không (text field) | Direct: `inventory_dimensions_dict={"vi_tri_kho": "Kệ A-03"}` |
| **Báo cáo tồn kho** | Stock Balance theo Batch → group màu | Stock Balance filter dimension (core hỗ trợ sẵn) |
| **Bùng nổ Batch** | Có (mỗi lô màu mới = 1 batch mới) | Giảm (batch chỉ tạo khi có lô vật lý mới, không tạo cho màu) |
| **Thêm thuộc tính mới** | Thêm field vào Batch | Thêm Inventory Dimension record (không code) |
| **Giá vốn (COGS)** | Theo batch (dao động theo từng lô) | Pool chung (ổn định, dễ dự báo) |
| **Theo dõi lô (traceability)** | Batch làm cả 2 việc: lưu màu + track lô | Batch chỉ track lô (đúng bản chất) |
| **Offcut Serial No** | Serial No chồng lên Batch | Serial No chồng lên Batch (giữ nguyên) |
| **al_trace_stage** | Trên Batch — giữ nguyên | Trên Batch — giữ nguyên |
| **Định giá bán** | LOOKUP TRA-GIA-NHOM (brand, màu) | LOOKUP TRA-GIA-NHOM (brand, màu) — không đổi |
| **Stock Reconciliation** | Cần custom (core chặn batch) | Cần override SR (Phương án E) — đã có giải pháp |
| **Độ phức tạp triển khai** | Thấp — chỉ cần Batch core | Trung bình — cần override SR periodic |
| **Performance với nhiều dimension (>3)** | N/A | Cần database index (xem 4.23.7.7) |

**Kết luận:** Batch-màu phù hợp cho doanh nghiệp vừa và nhỏ, số lượng màu ít, không cần quản lý vị trí kho. Inventory Dimension phù hợp cho doanh nghiệp lớn, nhiều màu, cần quản lý vị trí, cần báo cáo linh hoạt.

#### 4.23.7.4 Mô hình Hybrid — Batch (tracking) + Inventory Dimension (màu + vị trí)

Đây là phương án tối ưu nhất — kết hợp ưu điểm của cả Batch và Inventory Dimension:

```
┌────────────────────────────────────────────────────────────────────┐
│  MÔ HÌNH HYBRID                                                     │
│                                                                     │
│  Item: "NHOM_6063_40x40" (has_batch_no=1)                          │
│  Batch Wise Valuation: TẮT (use_batchwise_valuation=0)             │
│  → Valuation pool AVCO/FIFO = Item + Warehouse                      │
│                                                                     │
│  ┌──────────────────┐    ┌──────────────────────────────────────┐   │
│  │  BATCH (giữ)      │    │  INVENTORY DIMENSION (bổ sung)      │   │
│  │──────────────────│    │──────────────────────────────────────│   │
│  │ • al_trace_stage │    │ • "Màu sắc" → SLE.mau_sac            │   │
│  │ • al_source_project│   │ • "Vị trí kệ" → SLE.vi_tri_kho      │   │
│  │ • al_is_reserved  │    │ • (Có thể thêm: Độ dày, Bề mặt...)  │   │
│  │ • Serial No offcut│    │                                      │   │
│  │ • al_bin_location │    │  SLE có đủ:                          │   │
│  │   (text, để tra)  │    │  item_code + warehouse + batch_no    │   │
│  └──────────────────┘    │  + mau_sac + vi_tri_kho               │   │
│                           │  + actual_qty + valuation_rate(pool)  │   │
│                           └──────────────────────────────────────┘   │
│                                                                     │
│  Query tồn kho theo màu: filter SLE.mau_sac trực tiếp              │
│  Query tồn kho theo vị trí: filter SLE.vi_tri_kho trực tiếp       │
│  Query tồn kho theo lô: filter batch_no (giống hiện tại)           │
│  Query tồn kho theo (màu + vị trí + lô): AND cả 3                 │
└────────────────────────────────────────────────────────────────────┘
```
...
```python
# Inventory Dimension 1: "Màu sắc"
{
    "dimension_name": "Màu sắc",
    "reference_document": "AL Color Standard",  # DocType đã có
    "apply_to_all_doctypes": 1,
    "source_fieldname": "mau_sac",
    "target_fieldname": "mau_sac"  # → SLE.mau_sac
}

# Inventory Dimension 2: "Vị trí kệ"
{
    "dimension_name": "Vị trí kệ",
    "reference_document": "Vi tri kho",  # hoặc Location (DocType tự tạo)
    "apply_to_all_doctypes": 1,
    "source_fieldname": "vi_tri_kho",
    "target_fieldname": "vi_tri_kho"  # → SLE.vi_tri_kho
}

# Batch: tắt Batch Wise Valuation
# Item: has_batch_no=1, use_batchwise_valuation=0
```

**Ví dụ dữ liệu:**
```
SLE 1: item=NHOM_40x40, wh=Kho NVL, batch=LOT-001, mau_sac=Trắng, 
       vi_tri_kho=Kệ_A03, qty=500, rate=10.000 (pool)
SLE 2: item=NHOM_40x40, wh=Kho NVL, batch=LOT-002, mau_sac=Trắng,
       vi_tri_kho=Kệ_A05, qty=300, rate=10.750 (pool sau khi nhập LOT-002)
SLE 3: item=NHOM_40x40, wh=Kho NVL, batch=LOT-003, mau_sac=Đen,
       vi_tri_kho=Kệ_B01, qty=200, rate=11.000 (pool)

Query: get_stock_balance("NHOM_40x40", "Kho NVL", 
         inventory_dimensions_dict={"mau_sac": "Trắng"})
→ qty=800, rate=10.750
```

#### 4.23.7.5 Batch Wise Valuation: Bật hay Tắt trong Hybrid?

| Trường hợp | Batch Wise Valuation | Hệ quả |
|---|---|---|
| **Công ty nhỏ, ít màu, ít lô** | ✅ Giữ nguyên (bật) | Batch vừa track màu vừa có valuation riêng — đơn giản, không cần thêm dimension |
| **Công ty lớn, nhiều lô, giá nhôm biến động** | ❌ Tắt — dùng pool AVCO | Giá vốn ổn định, không dao động theo lô. Batch chỉ track. Cần Inventory Dimension cho màu |
| **Muốn vừa pool vừa batch-wise** | ⚠️ Không thể — core chỉ chọn 1 trong 2 | Chọn pool cho phần lớn vật tư, batch-wise cho vật tư đặc thù (kính cường lực...) |

> **Khuyến nghị:** Đa số công ty nhôm kính nên **tắt Batch Wise Valuation, dùng pool AVCO + Inventory Dimension cho màu sắc**. Chỉ bật batch-wise nếu thực sự cần theo dõi giá vốn riêng cho từng lô (VD: kính nhập khẩu giá trị cao, mỗi container giá khác nhau).

#### 4.23.7.6 Tác động đến các module khác khi dùng Hybrid

**Stock Entry / Purchase Receipt / Delivery Note:**
- Khi nhập kho: chọn `mau_sac` (dropdown từ AL Color Standard), `vi_tri_kho` (dropdown từ Location), `batch_no` (tạo mới hoặc chọn batch có sẵn)
- Core tự ghi 3 trường này vào SLE → không cần custom
- Batch vẫn nhận `al_trace_stage` update qua hook (giống §4.25)

**Stock Reconciliation:**
- Với batch: core chặn SR nếu batch có current_qty>0 → cần override (như mọi batch khác)
- Với dimension: core chặn SR nếu dimension có current_qty>0 → cần override Phương án E
- **Giải pháp chung:** Override SR → sinh Stock Entry (đã có giải pháp trong tài liệu ERPNext_Stock_Reconciliation_Inventory_Dimension_Giai_Phap_v5.md §14-18)

**Cutting Plan xuất kho:**
- Xuất NVL: ghi nhận `batch_no` + `mau_sac` + `vi_tri_kho` trên Stock Entry Detail
- Core tự động ghi cả 3 vào SLE
- Ưu tiên batch đã reserve cho project (§4.23.6) — không đổi

**Báo cáo tồn kho:**
- Stock Balance Report: filter `mau_sac`, `vi_tri_kho`, `batch_no` — linh hoạt hơn hiện tại
- Stock Ledger Report: filter tương tự
- Báo cáo custom cũ: nếu dùng query join Batch → `al_color`, vẫn chạy được (không phá)
- Báo cáo mới: query trực tiếp SLE.mau_sac (nhanh hơn)

**MRP Lite:**
- Nếu dùng Hybrid: tổng hợp theo `(item_code, mau_sac)` thay vì `(item_code, batch.al_color)` — không cần join Batch
- Nếu vẫn dùng Batch-màu: giữ nguyên logic cũ

#### 4.23.7.7 Yêu cầu kỹ thuật khi dùng Inventory Dimension

**Database Indexing (bắt buộc nếu dùng >2 dimension):**
```sql
CREATE INDEX idx_sle_item_dim 
ON `tabStock Ledger Entry` (item_code, warehouse, mau_sac, vi_tri_kho);
CREATE INDEX idx_sle_mau_sac ON `tabStock Ledger Entry` (mau_sac);
CREATE INDEX idx_sle_vi_tri_kho ON `tabStock Ledger Entry` (vi_tri_kho);
```

**Override Stock Reconciliation (bắt buộc):**
Xem giải pháp chi tiết tại tài liệu `ERPNext_Stock_Reconciliation_Inventory_Dimension_Giai_Phap_v5.md` — Phương án E (Mục 14). Tóm tắt: override `on_submit()` của Stock Reconciliation, rẽ nhánh:
- `Purpose = "Opening Stock"` → core pipeline (super)
- `Purpose = "Stock Reconciliation"` (periodic) → sinh Stock Entry tương ứng

**Số dimension tối đa khuyến nghị: 3-5 dimension.** Quá nhiều dimension ảnh hưởng performance do SLE phải ghi nhiều field hơn.

#### 4.23.7.8 Ma trận quyết định chọn phương án

| Điều kiện | A. Hybrid (mặc định) ✅ | B. Batch-màu (đơn giản hoá) |
|---|---|---|
| Số màu sắc quản lý | ≥ 20 hoặc không chắc chắn | < 20, ổn định |
| Cần quản lý vị trí kệ | ✅ Có | ❌ Không |
| Số giao dịch tồn kho/tháng | ≥ 5.000 | < 5.000 |
| Biến động giá nhôm thường xuyên | Pool AVCO hấp thụ tốt hơn | Batch-wise dễ bị COGS dao động |
| Cần báo cáo tồn kho theo thuộc tính | Filter trực tiếp SLE | Phải join Batch |
| Đã có sẵn ERPNext Custom App | ✅ (eupapp) — dễ override SR | Có thể không cần |
| Nguồn lực kỹ thuật | Có DBA, có Frappe dev | Ít hoặc thuê ngoài |
| Quy mô doanh nghiệp | Vừa / Lớn | Nhỏ / Vừa |

> **Nếu vẫn phân vân, hãy chọn Hybrid (mặc định).** Có thể fallback sang Batch-màu bằng cách tắt Inventory Dimension và bật Batch Wise Valuation — dữ liệu SLE cũ vẫn còn `mau_sac` nhưng không bắt buộc. Hướng ngược lại (Batch → Hybrid) phức tạp hơn vì phải backfill dữ liệu từ Batch.al_color vào SLE.mau_sac.

#### 4.23.7.9 Lộ trình nâng cấp từ Batch-màu lên Hybrid (nếu cần)

Nếu đang chạy Batch-màu và muốn thêm Inventory Dimension sau này:

```
Phase 0: Cấu hình Inventory Dimension (không ảnh hưởng dữ liệu cũ)
  → Core tự thêm field `mau_sac`, `vi_tri_kho` vào các DocType
  → SLE cũ vẫn có dữ liệu (batch_no, không có dimension mới)
  → Không phá gì

Phase 1: Đồng bộ dữ liệu batch → dimension (script 1 lần)
  UPDATE `tabStock Ledger Entry` sle
  JOIN `tabBatch` b ON b.name = sle.batch_no
  SET sle.mau_sac = b.al_color
  WHERE sle.mau_sac IS NULL AND b.al_color IS NOT NULL;

Phase 2: Chuyển báo cáo từ batch join → dimension filter
  → Báo cáo cũ giữ nguyên, báo cáo mới dùng filter mới

Phase 3: Override SR (nếu muốn dùng SR periodic với dimension)
  → Áp dụng Phương án E
```
...
## XX. MATERIAL PLANNING (MRP LITE) — THIẾT KẾ CHI TIẾT

### 20.1 Mục tiêu

Tự động tổng hợp nhu cầu vật tư từ các Sales Order đã Submit, đối chiếu với tồn kho hiện tại (kể cả offcut), và đề xuất đơn mua hàng (Purchase Order) hoặc đơn gia công (Subcontracting Order) theo từng dự án hoặc gom theo nhà cung cấp.

### 20.2 Kiến trúc dữ liệu

```
                  ┌─────────────────────────────────────┐
                  │  Sales Order (đã Submit)             │
                  │  → Item + qty + màu (từ BOM)        │
                  └──────────────┬──────────────────────┘
                                 │
                                 ▼
                  ┌─────────────────────────────────────┐
                  │  AL Material Plan (Master)           │
                  │  ├── project (Link → Project)        │
                  │  ├── from_date / to_date             │
                  │  ├── status (Draft/Ready/Ordered)    │
                  │  └── al_lines (Table → AL Material Plan Line)
                  └──────────────────────────────────────┘
                                 │
                                 ▼
                  ┌─────────────────────────────────────┐
                  ...
                  │  ├── item_code (Link → Item)          │
                  │  ├── color_code (Link → AL Color Std)│
                  │  ├── source (QUOTATION / MANUAL)     │
                  │  ├── source_sales_order              │
                  │  ├── required_qty (nhu cầu)          │
                  │  ├── available_stock_qty (tồn kho)   │
                  │  ├── available_offcut_qty (đầu thừa) │
                  │  ├── shortfall_qty (thiếu)           │
                  │  ├── suggested_action (BUY / SUBCONTRACT / MAKE)
                  │  ├── suggested_po_qty                │
                  │  ├── lead_time_days                  │
                  │  ├── preferred_supplier              │
                  │  ├── project (Link → Project)        │
...
### 20.3 DocType — AL Material Plan

| fieldname | fieldtype | Mô tả |
|---|---|---|
| plan_name | Data (autoname) | VD: MRP-20260710 |
| company | Link → Company | |
| from_date | Date | Đầu kỳ lập kế hoạch |
| to_date | Date | Cuối kỳ |
| status | Select | Draft / Ready / Ordered / Cancelled |
| include_standard_stock | Check | Bật → cộng tồn kho đệm an toàn (`default_safety_stock_qty`) |
| notes | Text | |
| al_lines | Table → AL Material Plan Line | |

### 20.4 DocType — AL Material Plan Line (Child)

| fieldname | fieldtype | reqd | Mô tả |
|---|---|---|---|
| item_code | Link → Item | ✓ | |
| color_code | Link → AL Color Standard | | Nếu Hybrid (mặc định): màu từ Inventory Dimension; nếu Batch-màu: màu từ Batch.al_color |
| item_name | Data | (read_only) | |
| source | Select | ✓ | QUOTATION / MANUAL |
| source_sales_order | Link → Sales Order | | Nếu source=QUOTATION |
| project | Link → Project | | Rỗng nếu là tồn kho đệm chuẩn (`is_standard_stock=1`) |
| required_qty | Float | ✓ | Tổng nhu cầu (từ SO + dự phòng) |
| available_stock_qty | Float (computed) | | Tồn kho hiện tại: nếu Hybrid = `get_stock_balance(item, wh, {mau_sac: color_code})`; nếu Batch-màu = SUM batch theo `al_color` |
| available_offcut_qty | Float (computed) | | Tồn kho Serial No có `al_is_offcut=1` cùng item + màu (chỉ áp dụng cho NHOM_PROFILE) |
| shortfall_qty | Float (computed) | | `required_qty - available_stock_qty - available_offcut_qty` (tối thiểu 0) |
| safety_stock_qty | Float | | Từ `AL Color Standard.default_safety_stock_qty` nếu `is_standard_stock=1` |
| adjusted_shortfall | Float (computed) | | `shortfall_qty + safety_stock_qty` |
| suggested_action | Select | | BUY / SUBCONTRACT / MAKE / NONE |
| suggested_po_qty | Float | | Đề xuất số lượng mua (làm tròn theo `stock_bar_length_mm` nếu NHOM_PROFILE) |
| lead_time_days | Int (computed) | | Tra cứu từ Lead Time Matrix (§20.7) |
| preferred_supplier | Link → Supplier | | Gợi ý từ Supplier Price List mới nhất |
| status | Select | | PENDING / ORDERED / CANCELLED |
| al_bin_location_hint | Data | | Gợi ý vị trí lưu kho mặc định |

### 20.5 Nguồn dữ liệu — tổng hợp nhu cầu

Khi user nhấn "Fetch Sales Orders", hệ thống:

```python
def fetch_sales_orders(plan):
    """Lấy tất cả Sales Order Item trong kỳ, phân tích nhu cầu vật tư"""
    so_items = frappe.db.sql("""
        SELECT 
            soi.item_code, soi.qty, soi.project,
            soi.al_mau_nhom,
            soi.al_bom, soi.al_config_snapshot
        FROM `tabSales Order Item` soi
        JOIN `tabSales Order` so ON so.name = soi.parent
        WHERE so.docstatus = 1
            AND so.transaction_date BETWEEN %(from)s AND %(to)s
            AND so.status NOT IN ('Closed', 'Cancelled')
    """, {"from": plan.from_date, "to": plan.to_date}, as_dict=1)
    
    # Với mỗi SO Item, đọc ConfigSnapshot → phân tích nhu cầu từng dòng BOM
    for item in so_items:
        bom_lines = get_bom_material_requirements(item.al_config_snapshot)
        for bom_line in bom_lines:
            plan.append_line(
                item_code=bom_line.item_code,
                color_code=item.al_mau_nhom,
                required_qty=bom_line.qty,
                source="QUOTATION",
                source_sales_order=item.parent,
                project=item.project,
            )
```

**ConfigSnapshot → phân tích nhu cầu:**
```python
def get_bom_material_requirements(config_snapshot_name):
    """Đọc ConfigSnapshot, trích xuất nhu cầu vật tư từ al_lines"""
    snapshot = frappe.get_doc("ConfigSnapshot", config_snapshot_name)
    bom_data = json.loads(snapshot.enterprise_snapshot_json)
    requirements = []
    for line in bom_data.get("al_lines", []):
        if line.get("al_item_type") in ("NHOM_PROFILE", "KINH", "VTP"):
            requirements.append({
                "item_code": line["item_code"],
                "qty": line.get("quantity_only", 0) or line.get("total_qty", 0),
                "uom": line.get("uom"),
            })
    return requirements
```

### 20.6 Logic tính tồn kho

**Nếu dùng Hybrid (Inventory Dimension — mặc định):**
```python
def get_available_stock(item_code, color_code, warehouse):
    """Tồn kho theo màu — filter SLE dimension trực tiếp"""
    bal = get_stock_balance(
        item_code, warehouse,
        inventory_dimensions_dict={"mau_sac": color_code}
    )
    return bal[0]  # qty
```

**Nếu dùng Batch-màu (phương án đơn giản hoá):**
```python
def get_available_stock_batch(item_code, color_code, warehouse):
    """Tồn kho theo màu — join Batch.al_color"""
    ...
    """, (item_code, warehouse, color_code), as_dict=True)[0].qty or 0
```

### 20.7 Lead Time Matrix

**DocType: AL Lead Time Matrix (Master Data, optional)**

| fieldname | fieldtype | Mô tả |
|---|---|---|
| supplier | Link → Supplier | |
| item_group | Link → Item Group | |
| country_of_origin | Data | VD: "Trung Quốc", "Việt Nam" |
| min_lead_time_days | Int | |
| max_lead_time_days | Int | |
| typical_lead_time_days | Int | Dùng cho MRP mặc định |

**Fallback nếu không có ma trận:** dùng `AL Project Financial Config.typical_lead_time_days` (mặc định 30 ngày cho nhôm, 14 ngày cho kính trong nước, 7 ngày cho VTP).

### 20.8 Offcut ưu tiên

Trước khi đề xuất mua mới, MRP phải **duyệt tồn kho offcut** (chỉ áp dụng cho NHOM_PROFILE):

```python
def get_available_offcut(item_code, color_code, warehouse, min_length_mm=300):
    """Tồn kho đầu thừa tái sử dụng được"""
    # Serial No trong batch có màu tương ứng
    offcuts = frappe.db.sql("""
        SELECT sn.name, sn.al_piece_length_mm
        FROM `tabSerial No` sn
        JOIN `tabBatch` b ON b.name = sn.batch_no
        WHERE sn.item_code = %s
            AND sn.warehouse = %s
            AND sn.al_is_offcut = 1
            AND sn.al_piece_length_mm >= %s
            AND b.al_color = %s
            AND sn.status = 'Active'
        ORDER BY sn.al_piece_length_mm ASC  -- ưu tiên khớp đúng kích thước
    """, (item_code, warehouse, min_length_mm, color_code), as_dict=True)
    
    total_length = sum(o.al_piece_length_mm for o in offcuts)
    return len(offcuts), total_length  # số mảnh, tổng chiều dài (mm)
```

Khi `available_offcut_qty` ≥ `required_qty` → không đề xuất mua mới. Khi thiếu → mua bù phần còn lại.

### 20.9 Gom đơn hàng (Consolidation)

| Tình huống | `is_standard_stock=1` | `is_standard_stock=0` |
|---|---|---|
| **Cùng item, cùng màu, khác dự án** | Gom chung vào 1 PO | Để riêng từng PO theo project |
| **Cùng supplier, khác item** | Gom chung PO | Gom chung PO (nếu cùng supplier) |
| **Cùng item, khác màu** | Để riêng từng dòng PO | Để riêng từng dòng PO |
| **Offcut available** | Trừ trước khi tính PO | Trừ trước khi tính PO |

```python
def consolidate_and_suggest_po(plan_lines):
    """Gom nhu cầu → đề xuất PO theo supplier"""
    suppliers = {}
    
    for line in plan_lines:
        if line.shortfall_qty <= 0:
            line.suggested_action = "NONE"
            continue
        
        # Xác định supplier ưu tiên
        supplier = get_preferred_supplier(line.item_code, line.color_code)
        
        # Gom theo supplier
        key = (supplier, line.project if not line.is_standard_stock else None)
        if key not in suppliers:
            suppliers[key] = []
        suppliers[key].append(line)
    
    # Tạo PO đề xuất cho mỗi supplier
    for (supplier, project), lines in suppliers.items():
        po_qty = sum(line.adjusted_shortfall for line in lines)
        # Làm tròn theo quy cách đóng gói
        po_qty = round_to_packing_unit(po_qty, lines[0].item_code)
        
        for line in lines:
            line.suggested_action = "BUY"
            line.suggested_po_qty = po_qty * (line.adjusted_shortfall / sum(l.adjusted_shortfall for l in lines))
            line.preferred_supplier = supplier
```

### 20.10 Luồng xử lý tổng thể

```
User click "Tạo Material Plan"
  │
  ├── Bước 1: Fetch Sales Orders → phân tích nhu cầu từ ConfigSnapshot
  ├── Bước 2: Đọc tồn kho (tồn thường + offcut) → tính shortfall
  ├── Bước 3: Tra Lead Time Matrix → gán lead_time_days
  ├── Bước 4: Đề xuất supplier từ Supplier Price List
  ├── Bước 5: Gom đơn hàng → đề xuất PO
  ├── Bước 6: User review → Approve → tạo Material Request / PO
  └── Bước 7: Khi PO nhập kho → cập nhật status = ORDERED
```

### 20.11 Tích hợp với các module khác

| Module | Liên kết | Hướng |
|---|---|---|
| **ConfigSnapshot** | Đọc nhu cầu vật tư từ BOM đã tính | MRP ← ConfigSnapshot |
| **AL Color Standard** | `is_standard_stock`, `default_safety_stock_qty` | MRP → AL Color Standard |
| **AL Supplier Price List** | Gợi ý supplier ưu tiên (giá rẻ nhất, còn hiệu lực) | MRP → Supplier Price List |
| **Stock Entry / Purchase Receipt** | Cập nhật `available_stock_qty` khi nhập | PR → MRP (realtime) |
| **Serial No (offcut)** | `al_is_offcut=1` được tính vào available | MRP ← Serial No |
| **Project** | Nếu `is_standard_stock=0` → PO gắn project | MRP → PO → Project |
| **Change Order** | Khi Change Order approved → cập nhật MRP | CO → MRP (re-run) |

### 20.12 Test Scenarios (bổ sung cho §XXXIII)

| # | Kịch bản | Kỳ vọng |
|---|---|---|
| M1 | SO 10 bộ cửa → MRP tính đúng nhu cầu nhôm + kính + VTP | Số liệu khớp với ConfigSnapshot |
| M2 | Tồn kho đã có 5 bộ → shortfall = 5 | Tính đúng |
| M3 | Có offcut đủ dùng → không đề xuất mua nhôm | Offcut ưu tiên |
| M4 | 2 SO cùng item, cùng màu, `is_standard_stock=1` → gom 1 PO | Gom đúng |
| M5 | 2 SO cùng item, cùng màu, `is_standard_stock=0` → 2 PO riêng | Không gom |
| M6 | Supplier Price List hết hạn → fallback supplier khác | Chọn đúng |
...
**Danh mục use case AI:** (xem Roadmap AI §29.1 để biết use case nào thuộc Phase nào)
...
### 29.1 ★ Roadmap AI theo Phase — giới hạn thực tế

> AI là một lĩnh vực riêng, không thể làm tất cả trong 1 phase. Dưới đây là roadmap được sắp xếp theo ROI và độ phụ thuộc dữ liệu. **Phase 1-2 chỉ tập trung vào 3 use case có ROI rõ nhất.**

| Phase | Use Case | Phụ thuộc dữ liệu | ROI | Ghi chú kỹ thuật |
|---|---|---|---|---|
| **Phase 1** | AI Quotation Copilot | Cần ConfigSnapshot từ Phase 0 | ⭐⭐⭐ Cao — giảm 30-50% thời gian lên báo giá | Gợi ý dòng BOM, phát hiện cấu hình thiếu ± kiểm tra giá so với lịch sử |
| **Phase 2** | AI Cutting Optimization | Cần Cutting Plan thật (Phase 2) | ⭐⭐⭐ Cao — tiết kiệm 5-15% nguyên vật liệu | Dùng `frappe.enqueue(queue="long")`, không block luồng chính |
| **Phase 2** | AI Trợ lý tri thức (RAG) | Frappe API + LLM, không cần training data | ⭐⭐⭐ Cao — hỗ trợ tra cứu quy trình, dữ liệu master | Dùng Frappe API hook, không cần thêm DocType |
| **Phase 2** | AI Config Validator | Cần dữ liệu master Phase 0 | ⭐⭐ Trung bình — phát hiện lỗi cấu hình sớm | Kiểm tra Rule LOOKUP có đủ key không, BOM có field thiếu không |
| **Phase 2** | AI Win/Loss Analysis | Cần `al_loss_reason` từ Phase 1 | ⭐⭐ Trung bình — cải thiện tỷ lệ thắng thầu | Chạy `daily_ai_analysis`, đọc `Quotation.al_loss_reason` |
| **Phase 3** | AI PK Suggester | Cần dữ liệu BOM từ Phase 1-2 | ⭐⭐ Trung bình | Gợi ý PK Set dựa trên mặt hàng tương tự |
| **Phase 3** | AI Formula/Rule Generator | Cần dữ liệu Rule từ Phase 0 | ⭐⭐ Trung bình | Sinh Rule/Formula từ mô tả tiếng Việt |
| **Phase 3** | AI Drawing-to-BOM | Cần dữ liệu BOM + Drawing thật | ⭐⭐⭐ Cao nhưng khó | Computer Vision — phải thuê/hợp tác AI chuyên sâu |
| **Phase 3** | AI Pricing/Negotiation Advisor | Cần lịch sử đàm phán (Phase 1-2) | ⭐⭐ Trung bình | Phân tích biên độ giảm giá theo khách hàng |
| **Phase 4+** | AI Purchasing/Price Forecast | Cần 12-18 tháng dữ liệu Supplier Price List | ⭐⭐ Trung bình | Dự báo xu hướng giá nhôm thế giới |
| **Phase 4+** | AI Voice-to-Progress | Cần mobile app + IoT | ⭐ Thấp — phức tạp | Thiết bị đầu cuối, speech-to-text |
| **Phase 4+** | AI Root Cause Clustering | Cần 12+ tháng Warranty Claim | ⭐⭐ Trung bình | Gom nhóm lỗi sản phẩm, phát hiện lỗi hệ thống |
| **Phase 4+** | AI Project Health Score | Cần dữ liệu nhiều dự án hoàn chỉnh | ⭐⭐ Trung bình | Tổng hợp tiến độ, chi phí, chất lượng |
| **Phase 4+** | AI Anomaly Detection | Cần dữ liệu vận hành 12+ tháng | ⭐⭐ Trung bình | Phát hiện giao dịch bất thường |

> **Hướng dẫn:** Phase 1-2 tập trung vào **Quotation Copilot, Cutting Optimization, RAG** — 3 use case có ROI cao nhất, không phụ thuộc AI bên ngoài, có thể triển khai ngay. **Phase 3+ cần có dữ liệu thực tế từ Phase 1-2 trước khi xây dựng.**
...
| **Phase 0.5** | **★ Pilot Master Data — BẮT BUỘC** — Nhập dữ liệu master thật từ khách hàng đầu tiên: 1 hệ profile (5-10 profile), 10 BOM thật, 20 AL Color Standard, AL PK Set thật. Chạy BomOrchestrator từ đầu đến cuối, validate kết quả với người dùng thật. Fix FormulaEngine nếu phát sinh lỗi. **Gate:** Phase 0.5 pass → mới bắt đầu Phase 1 |
...
### ERD TỔNG THỂ — SƠ ĐỒ QUAN HỆ 62 DOCTYPE

> **Ghi chú:** ERD dạng text dưới đây dùng ký hiệu:
> - `───` : 1-1 hoặc 1-nhiều
> - `◇───` : optional (0 hoặc nhiều)
> - `[Child]` : child table (Table MultiSelect)
> - `[Core]` : DocType ERPNext core (tái sử dụng, không custom)

#### Tầng 1 — Master Data (24 DocType)

```
AL Variable Group ─── AL Variable Set ─── [Child] AL Variable Set Detail
                                           ─── AL Variable Library

AL Glass Type ─── AL Glass Master ─── [Child] AL Glass Layer Line

AL Product Type

AL Material Type

AL Profile Set ─── [Child] AL Profile Line (trung tâm)

AL PK Set ─── [Child] AL PK Line

AL Cost Template ─── [Child] AL Cost Template Line

AL BOM ─── AL Variable Set
       ─── AL Profile Set
       ─── AL PK Set
       ─── AL Cost Template
       ─── [Child] ConfigSnapshot (audit trail)

AL Color Standard

AL Cutting Standard

AL Installation Team

AL Warranty Policy

AL Project Financial Config ─── AL Project Warehouse Map (optional)

AL Supplier Price List ─── [Child] AL Supplier Price List Line

AL Alert Config

Item [Core] ─── AL Color Standard (qua Inventory Dimension)
            ─── AL Cost Template
            ─── AL Glass Master

Batch [Core] ─── AL Color Standard (al_color)
            ─── Project (al_source_project)
```

#### Tầng 2 — Rule Engine & Variable Binding (10 DocType)

```
AL Calculation Rule ─── [Child] AL Rule Threshold Row
                    ─── [Child] AL Rule Lookup Row
                    ─── [Child] AL Rule Sequence Item

AL Dynamic Item Rule ─── [Child] AL Dynamic Item Threshold Row
                     ─── [Child] AL Dynamic Item Lookup Row
                     ─── AL Dynamic Item Rule Version (immutable)

Pricing Rule [Core]
Formula Variable Binding (formula_builder)
```

#### Tầng 4.5–5 — Orchestration & Presentation

```
BomOrchestrator (9 bước, gọi FormulaEngine từ formula_builder)
  ─── ConfigSnapshot (kết quả mỗi lần tính)
  ─── AL BOM Version (snapshot bất biến)

AL BOM Dialog / Quotation Item (custom fields)
```

#### Tầng 6 — Module Nghiệp Vụ (24 DocType)
...
**6B — Supply:**
```
AL Material Plan ─── [Child] AL Material Plan Line
                        ─── Item [Core]
                        ─── AL Color Standard
                        ─── Sales Order [Core]
                        ─── Project [Core]

AL Cost Variance ─── ConfigSnapshot
                 ─── AL Cost Bucket
                 ─── Project [Core]
...
**6C — Production:**
```
AL Production Order Bridge ◇─── Work Order [Core]
                           ◇─── Quality Inspection [Core]

AL Aluminum Cutting Plan ─── [Child] AL Aluminum Cutting Plan Line
                              ─── Serial No [Core] (offcut)
                              ─── Batch [Core] (al_trace_stage)

AL Glass Cutting Plan ─── [Child] AL Glass Cutting Plan Line
                    
AL Glass Cut Order ─── [Child] AL Glass Cut Line

AL Site Survey ─── [Child] AL Site Survey Line
                     ─── AL Design Revision (nếu lệch dung sai)
```

**6D — Field Operations:**
```
AL Installation Order ─── [Child] AL Installation Order Line
                           ─── Batch [Core] (al_trace_stage → INSTALLED)
                      ─── [Child] AL Installation Progress
                      ─── [Child] AL Installation Cost Actual
```

**6E — Financial:**
```
AL Change Order ─── [Child] AL Change Order Line
                     ─── AL BOM Version (new_bom_version)
                     ─── Sales Order [Core] (SO bổ sung)
                ◇─── AL Design Revision

AL Design Revision ─── [Child] AL Design Revision Line
                     ─── Sales Order Item [Core]
                     ─── AL Site Survey (nếu nguồn SITE_SURVEY_DEVIATION)
                ◇─── AL Change Order

AL Handover Acceptance

AL Project Profitability Snapshot ─── Project [Core]
                                 ─── ConfigSnapshot
                                 ─── GL Entry [Core] (actual cost, Nguyên tắc #31)
```

**6F — Tích hợp Core:**
```
Sales Order [Core] ◇─── ConfigSnapshot
                   ◇─── AL BOM Version
                   ◇─── AL Cost Variance
                   ◇─── Pricing Rule [Core]
                   ◇─── Payment Schedule [Core]

Sales Invoice [Core] ◇─── Payment Schedule [Core]
                     ◇─── AL Project Profitability Snapshot

Purchase Order [Core] ◇─── AL Supplier Price List
                      ◇─── Project [Core]

Quality Inspection [Core] ◇─── AL Production Order Bridge

Warranty Claim [Core] ◇─── AL Installation Order
                      ◇─── AL Warranty Policy
                      ◇─── Stock Entry [Core] (sửa chữa bảo hành)
```

#### Tầng 7 — AI Governance (2 DocType)

```
AL AI Suggestion Log   (mọi đề xuất AI cần Approval)
AL AI Interaction Log  (tra cứu/RAG, không cần duyệt)
```

---

### Cập nhật: Những mục đã được bổ sung vào v26

So với v25, v26 đã bổ sung:

| Mục | Bổ sung |
|---|---|
| **§4.23** | Hybrid Inventory Dimension làm mặc định, Batch-màu là phương án đơn giản hoá |
| **§4.23.7** | Thiết kế chi tiết Hybrid, so sánh, ma trận quyết định, lộ trình nâng cấp |
| **§XX** | MRP Lite thiết kế chi tiết: Lead Time Matrix, Offcut ưu tiên, Gom đơn hàng |
...
2. **Dữ liệu master thật** — §VI chỉ có mẫu minh họa; công ty thật cần import toàn bộ catalog (thường 50-200 profile × N brand) trước go-live, cần kế hoạch riêng.
3. **Ma trận phân quyền chi tiết** — §XXXII mới ở mức vai trò/quyền đặc thù theo prose; Frappe cần khai báo cụ thể Read/Write/Create/Submit/Cancel/Amend cho từng DocType × Role, nên làm thành 1 bảng riêng trước Phase 1.

**Bước tiếp theo:** giao tài liệu này cho đội dev, bắt đầu Phase 0 theo lộ trình §XXXIV; xử lý 3 việc còn lại (DoD, master data, ma trận phân quyền) song song trước khi Phase 0 kết thúc. Phase 0.5 Pilot (dữ liệu master thật, 1 khách hàng, 10 BOM) là bắt buộc trước Phase 1 — xem §XXXIV.
