...
def _resolve_doc_values(doctype, docname):
    """Resolve giá trị THỰC TẾ (để hiện preview trong autocomplete) — đọc
    TRỰC TIẾP từ Formula Variable Binding (FVB, source_type =
    "linked_doctype_field") thay vì hardcode tay doctype/field.

    KIẾN TRÚC: FVB là cơ chế TỔNG QUÁT sống trong app `formula_builder`
    (dùng chung mọi app, không riêng alumglass) — quyết định "biến X lấy
    từ field Y của doctype Z, liên kết qua field W trên doctype hiện tại".
    DỮ LIỆU binding thực tế (bản ghi FVB) là của alumglass, seed qua
    `patches/v28_9/seed_fvb_from_variable_library.py`. `bom_orchestrator.py`
    (engine tính giá THẬT) đã đọc đúng nguồn này qua `_resolve_fb_context()`
    từ trước — hàm này (chỉ phục vụ preview UI) trước đây hardcode lại 1
    bản sao riêng của CÙNG mapping đó, khiến 2 nơi phải sửa tay đồng thời
    mỗi khi đổi cấu hình. Giờ đọc thẳng FVB — 1 nguồn dữ liệu duy nhất,
    đảm bảo giá trị preview LUÔN khớp với giá trị engine thực sự dùng.

    Thêm 1 biến system-var mới lấy từ doctype liên kết khác = thêm 1 FVB
    record (source_type="linked_doctype_field") cho đúng applies_to_doctype
    — KHÔNG sửa file này, không hardcode tên doctype nào ở đây nữa.
    """
    vals = {}
    ...
    try:
        ...
        bindings = frappe.get_all(
            ...
            fields=["variable_name", "source_config"],
        )
        for b in bindings:
            try:
                cfg = frappe.parse_json(b.get("source_config") or "{}")
            except Exception:
                continue
            link_field = cfg.get("link_field")
            target_doctype = cfg.get("target_doctype")
            target_field = cfg.get("target_field")
            if not (link_field and target_doctype and target_field):
                continue
            linked_name = doc.get(link_field)
            if not linked_name:
                continue
            val = frappe.db.get_value(target_doctype, linked_name, target_field)
            if val is not None:
                vals[b["variable_name"]] = val
    except Exception:
        # R9 — KHÔNG nuốt exception im lặng: ghi log đủ context (doctype, docname,
        # traceback) để chẩn đoán khi resolve FVB lỗi.
...
def get_variable_set_for_bom(bom_code):
    ...
    #     Lớp 1: child table system_variables (linh động theo TỪNG record
    #     Profile System). Lớp 2 (fallback): field cố định, đọc qua FVB —
    #     dùng chung `_resolve_doc_values()` (không hardcode lại mapping
    #     field/tên biến riêng ở đây, tránh 2 nguồn dữ liệu cho cùng 1 sự thật).
    if profile_system_code:
        try:
            ...
            for vname, val in _resolve_doc_values("AL Bom Set", bom_set.name).items():
