...
def _after_install():
    """Install hook: Custom Fields → Roles & Permissions → Workflow → Print Format."""
    ...
    from alumglass.setup.setup_workflows import install_workflows
...
def _after_migrate():
    ...
    V6 P11: `install_workflows()` — site cài trước khi thêm Workflow này cũng
    cần được backfill Role + Workflow record qua `bench migrate`, không chỉ
    site cài mới. Hàm idempotent (bỏ qua Workflow đã tồn tại, không ghi đè
    tuỳ chỉnh của user qua Workflow Builder UI).
    """
    from alumglass.setup.custom_fields import install_all_custom_fields
    from alumglass.setup.custom_fields import remove_stale_custom_fields
    from alumglass.setup.setup_workflows import install_workflows
