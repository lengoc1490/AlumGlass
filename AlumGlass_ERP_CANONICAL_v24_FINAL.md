# AlumGlass ERP — ĐẶC TẢ HỢP NHẤT CUỐI CÙNG (CANONICAL v24 FINAL)

> **App:** `aluglass` (Frappe custom app trên ERPNext core) + phụ thuộc `formula_builder` (Formula Engine độc lập)
> **Phiên bản:** v24 FINAL — bản hợp nhất đầy đủ, tự thân (self-contained), không cần tham chiếu file khác. Đây là tài liệu duy nhất dùng để triển khai production.
...
> **Đánh dấu:** ★ = mới so với ERPNext/Frappe thuần. Các mốc lịch sử (v18…v23) không còn đánh dấu riêng lẻ trong bản FINAL này — mọi quyết định đã được hợp nhất thành 1 thiết kế thống nhất duy nhất.
